...
# Group-overlay retention curves (one figure per biological group)
...
The `optimize_CS()` function always saves an objective audit trail (`.rds` and `.tsv`) covering every sample. By default it draws a single group-overlay decision plot per group in which each domain's **median Primary SI** is marked with a dashed reference line; pass `detail_samples=` to additionally export the per-sample SI decay curves.
