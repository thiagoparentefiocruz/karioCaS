...
karioCaS implements a **Multi-Strategy Stability Index (SI)** engine. Instead of relying on rigid geometric formulas, it analyzes the taxa-retention decay curve to mathematically identify the optimal threshold (Primary SI). It offers several strategies via `method=`:
...
```{r optimize_cs}
# Calculate the Stability Index for all domains using the default Kneedle method
audit_df <- optimize_CS(project_dir = proj_dir, tax_level = "Species")
