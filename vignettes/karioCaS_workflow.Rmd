...
karioCaS implements the **Stability Index (SI)**, an adaptation of the Kneedle Algorithm. It analyzes the exponential decay curve of taxa retention and mathematically identifies the "elbow" (Primary SI). This is the precise point of maximum deflection—where the cost of increasing stringency (losing true taxa) eclipses the benefit (removing false-positive k-mer noise). It also identifies Secondary SIs to help separate closely related biological noise.

```{r optimize_cs}
# Calculate the Stability Index for all domains at the Species level
audit_df <- optimize_CS(project_dir = proj_dir, tax_level = "Species")

# View the mathematical assessment for the Primary SIs
head(audit_df[audit_df$SI_Type == "Primary_SI", c("Sample", "Domain", "CS", "Taxa_Count", "Distance")])
```

The `optimize_CS()` function saves an objective audit trail (`.rds` and `.tsv`) and generates PDFs highlighting the mathematical elbows on the decay curves.
