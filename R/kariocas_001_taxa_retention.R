...
.tr_domain_plot_a <- function(df_calc, dom, baseline_df, fmt_num) {
  ...
  df_dom     <- dplyr::filter(df_calc, .data$Domain == dom,
                              .data$Rank %in% plot_ranks)
  if (nrow(df_dom) == 0) return(plot_kariocas_empty(dom, "No Data"))
  ...
  cnt     <- function(r) { v <- base$Base_Taxa[base$Rank == r]; if (length(v) == 0) 0 else v }
  sub_str <- paste0(
    "Reads: ", fmt_num(total),
  ...
  ggplot2::ggplot(
    ...
    ggplot2::labs(title = dom, subtitle = sub_str,
...
.tr_prep_b_data <- function(df_calc, dom, r, leg_taxa, leg_reads, leg_total) {
  ...
  df_viz |>
    ...
    dplyr::mutate(
      ...
      Metric_Label = factor(
        .data$Metric_Label, levels = c(leg_taxa, leg_total, leg_reads)
      )
    )
}
...
.tr_domain_plot_b <- function(df_calc, dom, r,
  ...
  if (is.null(df_long)) return(plot_kariocas_empty(dom, "No Data"))
  df_viz  <- dplyr::filter(df_calc, .data$Domain == dom, .data$Rank == r)
  sub_str <- paste0(
...
.tr_process_sample <- function(df_proc, samp, DOMAINS, output_dir, log_msg) {
  ...
  .tr_save_plot_a(df_calc, baseline$baseline_df, DOMAINS, samp,
                  fmt_num, output_dir, log_msg)
