...
#' @noRd
.ocs_process_domain <- function(df_samp, dom, method, manual_toll, samp, log_msg) {
    ...
    if (nrow(stats_df) < 3) {
        ...
        return(list(
            plot = plot_kariocas_empty(dom, "Insufficient CS points"),
            audit = NULL
        ))
    }
    ...
    if (max_taxa == min_taxa) {
        ...
        return(list(
            plot = plot_kariocas_empty(dom, "Flat Curve (No elbow)"),
            audit = NULL
        ))
    }
    calc_df <- stats_df |>
