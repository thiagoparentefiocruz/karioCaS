...
.rst_load_tse <- function(project_dir, log_msg) {
  ...
  list(count_matrix = count_matrix, row_meta = row_meta,
...
.rst_load_audit <- function(project_dir, tax_level, configs, log_msg) {
  needs_audit <- any(
    vapply(configs, function(x) tolower(as.character(x$val)) %in%
             c("auto", "secondary"), logical(1))
  )
  if (!needs_audit) return(NULL)
  ...
  if (!is.null(audit_df)) {
    ...
    if (nrow(sub) > 0) {
      if (uv == "auto") {
        ...
        if (length(v) > 0) return(list(val = v[1], tag = "[SI: Primary]"))
      } else {
...
.rst_match_column <- function(samp, requested_val, available_suffixes, log_msg) {
  for (suf in available_suffixes) {
    ...
    if (suf_num == requested_val)           return(suf)
    if ((requested_val * 10) == suf_num)    return(suf)
    if (requested_val == (suf_num * 10))    return(suf)
  }
  log_msg(sprintf(
...
retrieve_selected_taxa <- function(project_dir,
  ...
  tryCatch({
    ...
    write(paste0("\nCRITICAL ERROR: ", e$message),
          file = file.path(project_dir, "logs",
