...
.txr_calc_resolution <- function(df_dom, parent_level, child_level, top_n) {
  ...
  if (length(top_parents) == 0) return(NULL)
  df_parent <- df_dom |>
    dplyr::filter(.data$Parent_Name %in% top_parents,
...
.txr_domain_plot <- function(df_dom, dom, parent_level, child_level,
  ...
  if (is.null(calc_df)) return(plot_kariocas_empty(dom, "No Data"))
    ...
    for (cs in CS_LIST) {
      ...
      plots <- stats::setNames(
        lapply(DOMAINS, function(dom) {
          ...
          .txr_domain_plot(df_dom, dom, parent_level, child_level,
                           top_n, setup$log_msg)
        }),
        DOMAINS
      )
      .txr_save_panel(plots, DOMAINS, samp, cs, parent_level,
